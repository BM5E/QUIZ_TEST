...
def create_shared_content(user_id, content_type, content_id, expires_days=None):
    ...
    shared_content.share_key = share_key
    shared_content.content_type = content_type
    shared_content.content_id = content_id
    shared_content.expires_at = expires_at
    shared_content.is_public = True
...
def get_absolute_share_url(share_key):
    ...
    if base_url:
        relative_url = url_for('main.view_shared', share_key=share_key, _external=False)
        return urllib.parse.urljoin(base_url, relative_url)
    else:
        # Construct URL from request
        return url_for('main.view_shared', share_key=share_key, _external=True)
