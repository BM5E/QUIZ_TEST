...
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY", "missing_api_key")
client = None

# Only initialize the client if we have a valid API key
if OPENAI_API_KEY != "missing_api_key":
...
def get_fallback_quiz_questions(book_or_author, search_type, num_questions=5, question_type='choice'):
    ...
    author_questions = [
        ...
        {
            "question": "Many authors use a _____, which is a pen name different from their real name.",
            "question_type": "blank",
            "correct_answer": "pseudonym"
        },
        {
            "question": "The time period in which an author lives often influences their _____ and themes.",
            "question_type": "blank",
    ...
    # Customize questions with the book or author name where possible
    for q in questions:
        if search_type == 'book':
            q["question"] = q["question"].replace("in literature", f"in '{book_or_author}'")
        else:
            q["question"] = q["question"].replace("an author", f"the author {book_or_author}")
    
    # Filter by question type if needed
    if question_type != 'mixed':
        questions = [q for q in questions if q["question_type"] == question_type]
    
    # If we don't have enough questions, duplicate some (this is a fallback after all)
